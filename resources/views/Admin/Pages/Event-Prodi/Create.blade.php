        ...
        <div class="row g-0">
            ...
            <div class="col-md-7">
                <div class="card-header pb-0 text-center bg-transparent">
                    <h3 class="font-weight-bolder text-info text-gradient">Form Dokumentasi Kegiatan Prodi</h3>
                    <p class="mb-0">Lengkapilah Form Berikut Ini Secara Lengkap</p>
                </div>
                <div class="card-body">
                    <form action="{{ url('/admin/event-prodi/create') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="mb-3">
                            <select class="form-control" id="prodi_id" name="prodi_id">
                                <option value="">-- Pilih Prodi Yang Melaksanakan Kegiatan --</option>
                                @foreach ($kaprodi as $data_prodi)
                                <option value="{{ $data_prodi->id }}"
                                    {{ old('data_prodi_id') == $data_prodi->id ? 'selected' : '' }}>
                                    {{ $data_prodi->prodi }}
                                </option>
                                @endforeach
                            </select>
